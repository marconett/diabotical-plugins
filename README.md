...
4. Run the installer: **Windows:** double-click `install.bat`. **Linux (Proton):** run `./install.sh` from a terminal in that folder.
5. Start the game normally. If everything worked, a line starting with `[patchkit] patches loaded:` will appear in the console after launching the game.
